...
def accessTweeter(request):
    ...
    if tokenString:
        ...
        # make entry in userAccount table
            userAccount = CreateUserAccount(request.user,ACCOUNT_TWITTER,oauthAccess.mUser.GetId(),
                                            oauthAccess.mOauthAccessToken.to_string(),
                                            oauthAccess.mUser.AsJsonString(),oauthAccess.mUser.GetProfileImageUrl(),
                                            oauthAccess.mUser.GetScreenName(),True,0)
    ...
    if 'code' in request.GET:
        ...
        if resp.status == 200:
            ...
            try:
                ...
                userAccount = CreateUserAccount(request.user,ACCOUNT_INSTAGRAM,contentJson['user']['id'],
                                                contentJson['access_token'],content,contentJson['user']['profile_picture'],
                                                contentJson['user']['username'],True,1)
...
def accessFacebook(request):
    if 'code' in request.GET:
        ...
        if resp.status==200:
            ...
            userAccount = CreateUserAccount(request.user,ACCOUNT_FACEBOOK,userinformation['id'],
                                            accesstoken,userinfo,userPicture['content-location'],
                                            userinformation['username'],True,0)
    ...
    if 'code' in request.GET:
        ...
        if resp.status==200:
            ...
            try:
                ...
                userAccount = CreateUserAccount(user,ACCOUNT_FACEBOOK,userinformation['id'],accesstoken,
                                                userinfo,userPicture['content-location'],userinformation['username'],0)
                userAccount.is_active = True
